...
- `cobrowse_sessions`: consent-based cobrowsing attempts tied to a conversation, site, and visitor. Early connection telemetry is kept in `metadata.telemetry`, and the latest passive page state is kept in `metadata.page_state`, while the transport shape is still changing.
...
- Cobrowsing state is separate from conversations because consent, start, end timing, connection telemetry, and visitor page state need their own lifecycle.
