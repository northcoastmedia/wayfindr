...
- `cobrowse_sessions`: consent-based cobrowsing attempts tied to a conversation, site, and visitor. Early connection telemetry is kept in `metadata.telemetry`, the latest passive page state is kept in `metadata.page_state`, and the latest sanitized DOM snapshot is kept in `metadata.snapshot`, while the transport shape is still changing.
...
- Cobrowsing state is separate from conversations because consent, start, end timing, connection telemetry, visitor page state, and sanitized page snapshots need their own lifecycle.
