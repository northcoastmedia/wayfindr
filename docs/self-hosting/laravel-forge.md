...
- Initial deploy branch: `feat/agent-conversation-inbox` until the stack is
  merged to `main`.
- PHP: 8.3 or newer.
- Database: Postgres.
- Cache/queue: Redis.
...
2. Create the site and point the web directory to `apps/server/public`.
3. Connect the GitHub repository and deploy `feat/agent-conversation-inbox`.
4. Add the environment values above in Forge.
5. Enable TLS before testing the widget from another origin.
6. Enable the deployment health check against `/health`.
7. Add the queue worker and scheduler.
8. Run the first deploy.
9. Log in with the seeded demo agent only if the seeder has been run:
